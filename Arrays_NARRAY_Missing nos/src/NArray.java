...
public class NArray {
	...
	//Take a boolean array which is initialize as false. Change it to true while parsing input array
	// IF it is true means u got a duplicate element.
		public ArrayList<Integer> repeatedNumber(final List<Integer> a) { 
			...
			for(Integer num : a) {
				int index = num - 1;
				if(boolArr[index] == false)
					boolArr[index] = true;
				else
					result.add(num);
			}
			
			//Parse the boolean array once again to find which index is still false.
			int i = 0;
			for(Boolean input : boolArr) {
	...
	// DO NOT MODIFY THE LIST
		//Create an array with elements from 1 to n and remove the element 
	public ArrayList<Integer> repeatedNumber_02(final List<Integer> a) {
		ArrayList<Integer> result = new ArrayList<Integer>();
		...
		for (int i = 0; i < a.size(); i++) {
			int no = a.get(i);
			if(temp.contains(no)){ // If the no is available then remove it from temp arraylist
				int index = temp.indexOf(no);
				temp.remove(index);
			}
			else
				result.add(no); // This is the duplicate no.
		}
		result.add(temp.get(0)); // it is the last element whcih is left and this is the missing no.
...
//		Integer[] arr = {3, 1, 2, 5, 3};
		...
		System.out.println(n.repeatedNumber(arrlist));
	}	
}
